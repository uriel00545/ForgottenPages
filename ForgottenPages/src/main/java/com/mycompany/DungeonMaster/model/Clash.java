...
import com.mycompany.DungeonMaster.util.CalculaMoeda;
...
public class Clash {
    ...
    // parâmetros agora batem com os nomes usados dentro do método
    public String aplicar(Personagem usuarioAtk, Personagem usuarioDef) {
        ...
        switch (resultado) {
            ...
            case EMPATE -> {
                ...
                usuarioAtk.setHp(Math.max(0, usuarioAtk.getHp() - dano));
                usuarioDef.setHp(Math.max(0, usuarioDef.getHp() - dano)); // getHp(), não getDef()
